	...
	for _, s := range p.Services {
		...
		for _, c := range s.Characteristics {
			...
			if "ef6803019b3549339b1052ffa9740042" == c.UUID.String() {
				cln.WriteCharacteristic(c, []byte{0x01, 0xff, 0x00, 0x00}, true)
				fmt.Printf("write red\n")
				time.Sleep(3 * time.Second)
				cln.WriteCharacteristic(c, []byte{0x01, 0xff, 0xff, 0x00}, true)
				fmt.Printf("write green\n")
				time.Sleep(3 * time.Second)
				cln.WriteCharacteristic(c, []byte{0x01, 0x00, 0xff, 0x00}, true)
				fmt.Printf("write blue\n")
			}
